...
# Установим по умолчанию размер фигуры для Matplotlib.
matplotlib.rcParams['figure.figsize'] = [9, 6]
train_data, val_data, test_data = tfds.load("mnist",
                                            split=['train[10000:]', 'train[0:10000]', 'test'],
                                            batch_size=128, as_supervised=True)
x_viz, y_viz = tfds.load("mnist", split=['train[:1500]'], batch_size=-1, as_supervised=True)[
...
def preprocess(x, y):
    ...
    -1, 784))  # -1 - значит сохранить форму для заданного индекса матрицы, равносильно указать 1500
    # Масштабируем данные
    x = x / 255
    return x, y
...
train_data, val_data = train_data.map(preprocess), val_data.map(
    preprocess)  # map выполняет функцию preprocess для каждого элемента данных, к которым применяется

# ReLU
x = tf.linspace(-2, 2, 201)  # Тензорный вариант для linspace
x = tf.cast(x,
            tf.float32)  # Приведение тензора к нужному типу, в данном случае к 32-х разрядному числу с плавающей точкой
...
print(f"Время обучения модели: {elapsed_time} секунд")

# Функция построения графика, чтобы визуализировать потери и точность модели во время обучения
...
""""Загрузка модели и использование не видимых ранее моделью тестовых данных"""
...
for label in range(10):
  ...
  # Получение прогнозов для конкретной истинной метки
  pred_label = test_classes[label_ind]
  labels = y_test[label_ind]
  # Вычисление классовой точности
  label_accs[accuracy_score(pred_label, labels).numpy()] = label
for key in sorted(label_accs):
  print(f"Цифра {label_accs[key]}: {key:.3f}")

"""Визуализация ошибок посредство библеотеки Seaborn и Sklearn"""
